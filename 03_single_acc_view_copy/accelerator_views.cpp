...
int main(){
  float tm;
  {
    ...
    auto device_ptr = hc::am_alloc(size * sizeof(double), acc, 0);
    auto device_data = hc::array<double, 1>(extent<1>(size), acc_view, device_ptr);
    ...
    SHOW_TIME(acc_view.copy_async(host_data.data(), device_data.accelerator_pointer(), size * sizeof(double)));
    SHOW_TIME(busywork(device_data, 1, 0.0, 1.0));
    SHOW_TIME(acc_view.copy_async(device_data.accelerator_pointer(), host_data.data(), size * sizeof(double)));
